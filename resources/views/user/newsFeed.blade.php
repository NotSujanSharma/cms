                        ...
                        @if($club->events->count() > 0)
                            @foreach($club->events as $event)
                                <div class="bg-white p-4 rounded-xl border flex flex-col gap-2 shadow-md">
                                    ...
                                    <img src="{{ $event->picture_url }}" alt="{{ $club->name }}" class="w-full h-[300px] rounded-xl object-cover cursor-pointer" onclick="window.location.href='{{route('event.show', $event->id)}}'">
